...
export function AdherenceCard({ value, label = "Adherence", badge = "On track", className }: AdherenceCardProps) {
  return (
    <div className={cn("rounded-lg border border-line bg-surface p-5", className)}>
      <div className="flex items-start justify-between">
        <IconBadge icon={CheckCircle2} tone="accent" />
        <Badge variant="success" size="sm"><TrendingUp className="h-3.5 w-3.5" /> {badge}</Badge>
      </div>
